pipeline {
  ...
  environment {
    // Secrets (create these in Jenkins > Manage Credentials)
  ...
  stages {
    ...
    stage('Test') {
      steps { sh 'cd backend && npm test -- --ci --coverage' }
      post {
        always {
    ...
    stage('Deploy')      { steps { echo 'Deploy to staging (e.g., AWS EB/ECS) goes here' } }
    stage('Monitoring')  { steps { echo 'Run health checks / ping agents' } }
